...
class ProfileWrapper extends React.Component {
          ...
          <CollapsibleSegment title={v} key={k}>
            <Table
              ...
              rows={[
                this.successRowMatches(
                  v,
                  matches[k].liberal.events,
                  matches[k].liberal.successes,
                  matches[k].fascist.events,
                  matches[k].fascist.successes
                ),
              ]}
            />
          </CollapsibleSegment>
        ...
        rows={[
          this.successRow(
            <Popup
              inverted
              position="top center"
              content={voteAccuracyTooltip}
              trigger={<span style={{ cursor: "help" }}>Vote Accuracy</span>}
            />,
          ...
          this.successRow(
            <Popup
              inverted
              position="top center"
              content={shotAccuracyTooltip}
              trigger={<span style={{ cursor: "help" }}>Shot Accuracy</span>}
            />,
  ...
  Badges() {
    ...
    badgesToSort = badgesToSort.sort((a, b) =>
      this.state.badgeSort === "badge"
        ? compareID(a.id, b.id)
        : compare(new Date(a.dateAwarded), new Date(b.dateAwarded)) || compareID(a.id, b.id)
    );
        ...
        {badgesToSort.map((x) => (
          <>
            <img
              ...
              src={
                x.id === "xp10"
                  ? "../images/rainbowLarge.png"
                  : `../images/badges/${x.id.startsWith("eloReset") ? "eloReset" : x.id}.png`
              }
              alt={x.title}
              key={x.id}
              height={50}
            ...
            {x.id.startsWith("eloReset") ? (
              <p
                style={{ position: "relative", top: "50%", transform: "translateY(-100%)", display: "inline-block" }}
                key={x.id + "p"}
              >
                {x.id.substring(8)}
              </p>
        ...
        {this.props.profile.lastConnectedIP && (
          <div>
            <CollapsibleSegment title={"Private Info"} defaultExpanded={true}>
              <Table
                headers={["Last Connected IP / Signup IP"]}
                rows={[[this.props.profile.lastConnectedIP], [this.props.profile.signupIP]]}
              />
            </CollapsibleSegment>
          </div>
        )}
  ...
  Profile() {
    ...
    const userAdminRole =
      this.props.profile.staffRole === "admin" ||
      this.props.profile.staffRole === "editor" ||
      this.props.profile.staffRole === "moderator"
      ...
      <div>
        ...
        <div className="ui grid">
          ...
          {profile.playerPronouns && profile.playerPronouns !== "" && (
            <span
              className="ui pronouns eight wide column"
              style={{ paddingLeft: 0, paddingTop: 0, fontStyle: "italic", fontWeight: 700 }}
            >
  ...
  render() {
    ...
    const blacklist =
      profile._id !== this.props?.userInfo?.userName
        ? this.props?.profile?.blacklist
        : this.props?.userInfo?.gameSettings?.blacklist;
    ...
    return (
      <section id="profile" className="ui segment">
        ...
        >
          <div className="ui blacklist header">
            {this.props?.profile?._id !== this.props?.userInfo.userName ? "Player's Blacklist" : "Your Blacklist"}
          </div>
          {blacklist && (
