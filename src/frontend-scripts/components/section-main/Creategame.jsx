...
export default class Creategame extends React.Component {
  ...
  renderGameTypeDropdown() {
    const options = [
      ...
      },
      {
        ...
        label: (
          <>
            <span title="A casual game, gameplay rules are not enforced, does not count for winrate or Elo">
              Casual
            </span>
          </>
        ),
      },
      {
        ...
        label: (
          <>
            <span title="A practice game, gameplay rules ARE enforced, does not count for winrate or Elo">
              Practice
            </span>
          </>
        ),
      },
      {
        ...
        label: (
          <>
            <span title="A game with custom gameplay settings, gameplay rules are not enforced, does not count for winrate or Elo">
              Custom
            </span>
          </>
        ),
    ...
    const findValue = (val) => {
      ...
      if (val === "private") {
        return {
          ...
          label: (
            <>
              <span title="A private game with a password, gameplay rules are not enforced, does not count for winrate or Elo">
                Private
              </span>
            </>
          ),
  ...
  createNewGame = () => {
    ...
    if (this.state.containsBadWord) {
      ...
      const excludedPlayerCount = this.state.checkedSliderValues
      ...
      const data = {
        ...
        excludedPlayerCount,
        maxPlayersCount: customGameSettings.enabled
          ? customGameSliderValue[0]
          : this.state.isTourny
            ? undefined
  ...
  renderPlayerSlider() {
    ...
    const sliderCheckboxClick = (index) => {
      ...
      if (
        includedPlayerCounts.some((p) => p <= 6) &&
        this.state.monarchistSH &&
        this.state.avalonSH &&
        this.state.withPercival
      ) {
        newState.monarchistSH = false;
      }
      this.setState(newState);
  ...
  timedSliderChange = (timedSliderValue) => {
    this.setState((prevState) => ({
      timedSliderValue,
      gameType: timedSliderValue[0] < 30 ? "casual" : this.state.gameType,
    }));
  };
          ...
          {this.state.isEloLimited && (
            <div>
              ...
              <input
                ...
                style={{
                  background: `${this.state.eloSliderValue[0] < 1600 || this.state.eloSliderValue[0] > max ? "#f66" : "white"}`,
                  marginTop: "30px",
                }}
              />
            </div>
        ...
        <div className="sixteen wide column" style={{ marginTop: "-30px" }}>
          {this.state.isXPLimited && (
            <div>
              ...
              <Range
                onChange={this.xpSliderChange}
                min={0}
                max={playerXP}
                defaultValue={[1600]}
                value={this.state.xpSliderValue}
                marks={marks}
              />
              ...
              <input
                ...
                style={{
                  background: `${this.state.xpSliderValue[0] < 0 || this.state.xpSliderValue[0] > playerXP ? "#f66" : "white"}`,
                  marginTop: "30px",
                }}
              />
            </div>
      ...
      .map((val, i) => {
        return (
          <div
            key={`L${i}`}
            className="deckcard"
            style={{ backgroundImage: "url('../images/cards/liberalp-l.png')" }}
          />
        ); // eslint-disable-line
      })
      .concat(
        _.range(0, numFas).map((val, i) => {
          return (
            <div
              key={`F${i}`}
              className="deckcard"
              style={{ backgroundImage: "url('../images/cards/fascistp-l.png')" }}
            />
          ); // eslint-disable-line
        })
      );
  ...
  renderCustomGames() {
    ...
    const renderFas = () => {
      return _.range(0, this.state.customGameSettings.fascistCount).map((val, i) => (
        <div
          ...
          className="rolecard"
          style={{
            backgroundImage: hitKnowsFas
              ? `url('../images/cards/fascist${val}.png')`
              : "url('../images/cards/secretrole.png')",
          }} // eslint-disable-line
        />
      ));
    };
    ...
    const renderLib = () => {
      return _.range(0, this.state.customGameSliderValue - this.state.customGameSettings.fascistCount - 1).map(
        (val, i) => (
          <div
            key={i}
            className="rolecard"
            style={{
              backgroundImage: hitKnowsFas
                ? `url('../images/cards/liberal${val % 6}.png')`
                : "url('../images/cards/secretrole.png')",
            }} // eslint-disable-line
          />
        )
    ...
    return (
      <React.Fragment>
        ...
        <div
          className="eight wide column ui grid"
          style={{ marginTop: "-1rem", marginLeft: "3rem", marginBottom: "3rem" }}
        >
          <div className="row">
        ...
        <div className="ui grid">
          ...
          <div className="row">
            ...
            <div className="five wide column gamename">
              ...
              {this.state.containsBadWord && (
                <p className="contains-bad-word">This game name has a banned word or word fragment.</p>
              )}
            </div>
          ...
          <div className="row rebalance">
            {!this.state.customGameSettings.enabled && this.renderRebalanceCheckboxes()}
          </div>
              ...
              if (isRainbow) {
                return (
                  <div className="four wide column experiencedmode">
                    ...
                    <h4 className="ui header">
                      Rainbow game - only fellow 10+ exp veterans can be seated in this game
                    </h4>
                    <Switch
                      className="create-game-switch"
                      onChange={(checked) => {
          ...
          <div className="row">
            ...
            {!this.state.isTourny && (
              <div className="four wide column">
                ...
                />
                <h4 className="ui header">
                  Disable observer chat during game{this.state.disableobserverlobby ? "" : " only"}
                </h4>
                <Switch
                  className="create-game-switch"
                  onChange={(checked) => {
