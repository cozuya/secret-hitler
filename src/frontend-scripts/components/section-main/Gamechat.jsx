...
class Gamechat extends React.Component {
	...
	gameChatStatus() {
		...
		const isStaff = Boolean(userInfo.staffRole && userInfo.staffRole.length && userInfo.staffRole !== 'contrib' && userInfo.staffRole !== 'trial');
	...
	render() {
		...
		const isStaff = Boolean(userInfo && userInfo.staffRole && userInfo.staffRole.length && userInfo.staffRole !== 'contrib' && userInfo.staffRole !== 'trial');
		const hasNoAEM = players => {
			if (!userList || !userList.list) return false;
			return userList.list.every(user => {
				if (players.includes(user.userName) && user.staffRole && user.staffRole.length > 0 && user.staffRole !== 'contrib' && user.staffRole !== 'trial') return false;
				else return true;
			});
		};
