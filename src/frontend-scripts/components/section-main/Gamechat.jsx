...
class Gamechat extends React.Component {
	...
	gameChatStatus() {
		...
		const isStaff = Boolean(userInfo.staffRole && userInfo.staffRole.length && userInfo.staffRole !== 'trial');
	...
	processChats() {
		...
		if (isReplay || !gameInfo.general.private || userInfo.isSeated || isStaff) {
			return gameInfo.chats
				...
				.filter(
					chat =>
						(chatFilter !== 'Game' && chat.staffRole && chat.staffRole !== '' && chat.staffRole !== 'trial') ||
				...
				.map((chat, i) => {
					...
					const isMod = playerListPlayer && playerListPlayer.staffRole && playerListPlayer.staffRole !== '' && playerListPlayer.staffRole !== 'trial';
	...
	render() {
		...
		const isStaff = Boolean(userInfo && userInfo.staffRole && userInfo.staffRole.length && userInfo.staffRole !== 'trial');
		const hasNoAEM = players => {
			if (!userList || !userList.list) return false;
			return userList.list.every(user => {
				if (players.includes(user.userName) && user.staffRole && user.staffRole.length > 0 && user.staffRole !== 'trial') return false;
				else return true;
			});
		};
