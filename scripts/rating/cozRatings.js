...
function avg(accounts, players, accessor, fallback) {
	return (
		players.reduce(
			(prev, curr) =>
				(accessor(accounts.find((account) => account.username === curr))
					? accessor(accounts.find((account) => account.username === curr))
					: fallback) + prev,
			0
		) / players.length
...
async function rate(game) {
	...
	const averageRatingWinners =
		avg(accounts, winningPlayerNames, (a) => a.eloOverall, 1600) + b * libAdjust[game.playerCount];
	const averageRatingLosers =
		avg(accounts, losingPlayerNames, (a) => a.eloOverall, 1600) - b * libAdjust[game.playerCount];
	const averageRatingWinnersSeason =
		avg(accounts, winningPlayerNames, (a) => a.eloSeason, 1600) + (1 - b) * libAdjust[game.playerCount];
	const averageRatingLosersSeason =
		avg(accounts, losingPlayerNames, (a) => a.eloSeason, 1600) - (1 - b) * libAdjust[game.playerCount];
	// Elo Formula
	const k = 64;
