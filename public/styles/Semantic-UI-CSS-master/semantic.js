		...
		$allModules.each(function() {
			...
			module = {
				...
				toggle: function(query) {
					var $activeTitle =
							query !== undefined ? (typeof query === 'number' ? $title.eq(query) : $(query).closest(selector.title)) : $(this).closest(selector.title),
					...
					keydown: function(event) {
						...
						if (isShortcutKey) {
							var $currentlySelected = $item
								...
								inVisibleMenu =
									$parentMenu.hasClass(className.visible) || $parentMenu.hasClass(className.animating) || $parentMenu.parent(selector.menu).length > 0,
								hasSubMenu = $subMenu.length > 0,
								hasSelectedItem = $selectedItem.length > 0,
								selectedIsSelectable = $selectedItem.not(selector.unselectable).length > 0,
								delimiterPressed = pressedKey == keys.delimiter && settings.allowAdditions && module.is.multiple(),
								isAdditionWithoutMenu =
									settings.allowAdditions && settings.hideAdditions && (pressedKey == keys.enter || delimiterPressed) && selectedIsSelectable,
								$nextItem,
								isSubMenuItem,
								newIndex;
							...
							if (module.is.visible()) {
								...
								if (pressedKey == keys.upArrow) {
									$nextItem =
										hasSelectedItem && inVisibleMenu ? $selectedItem.prevAll(selector.item + ':not(' + selector.unselectable + ')').eq(0) : $item.eq(0);
