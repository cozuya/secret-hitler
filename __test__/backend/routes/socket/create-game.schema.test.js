import { createGameSchema, customGameSettingsSchema, updateWhitelistSchema } from '../../../../routes/socket/user-events/create-game.schema';
...
describe('customGameSettingsSchema', () => {
	...
	it('enforces deck/track ranges', () => {
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), deckState: { lib: 9, fas: 11 } }).success).toBe(false); // lib > 8
	...
	it('requires vetoZone > trackState.fas', () => {
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), trackState: { lib: 0, fas: 5 }, vetoZone: 5 }).success).toBe(false);
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), trackState: { lib: 0, fas: 3 }, vetoZone: 4 }).success).toBe(true);
	});

	it('validates the powers array (length 5, known powers or null)', () => {
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), powers: [null, null, null, null] }).success).toBe(false); // length 4
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), powers: ['banana', null, null, null, null] }).success).toBe(false);
		const res = customGameSettingsSchema.safeParse({ ...validCustom(), powers: ['investigate', '', 'null', 'bullet', null] });
		expect(res.success).toBe(true);
	...
	it('rejects object-injection on numeric fields', () => {
		...
		expect(customGameSettingsSchema.safeParse({ ...validCustom(), deckState: { lib: {}, fas: 11 } }).success).toBe(false);
	});
});
