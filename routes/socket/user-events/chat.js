...
module.exports.handleNewGeneralChat = async (socket, passport, data, modUserNames, editorUserNames, adminUserNames) => {
  ...
  const AEM =
    user.staffRole && user.staffRole !== "altmod" && user.staffRole !== "trialmod" && user.staffRole !== "veteran";
  ...
  if (pingMods) {
    try {
      ...
      if (!lastModPing || Date.now() > lastModPing + 180000) {
        ...
        socket.emit(
          "sendAlert",
          `You can't ping mods for another ${(lastModPing + 180000 - Date.now()) / 1000} seconds.`
        );
      }
    } catch (err) {
      console.error(err);
