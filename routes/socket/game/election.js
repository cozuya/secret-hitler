...
module.exports.selectVoting = data => {
	...
	function failedElection () {
		...
		if (game.trackState.electionTrackerCount === 3) {
			...
			// todo-alpha prevent policy powers from happening, clear prev elected governemnt (per rules)
...
function enactPolicy (game, team) {
	...
	setTimeout(() => {
		...
		if (powerToEnact) {
			...
			game.private.unSeatedGameChats.push(chat);
			powerToEnact[0](game);
		} else if (game.trackState.liberalPolicyCount === 5 || game.trackState.fascistPolicyCount === 1) {
