...
const { specialElection, policyPeek, investigateLoyalty, executePlayer } = require('./policy-powers.js');
...
module.exports.selectChancellor = data => {
	...
	if (!game.private.lock.selectChancellor) {
		...
		// 	const nonVotedPlayerNames = seatedPlayers.filter(player => !player.voteStatus.hasVoted).map(player => player.userName);
		// 	nonVotedPlayerNames.forEach(userName => {
		// 		selectVoting({
		// 			vote: true,
		// 			userName,
