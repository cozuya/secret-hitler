...
const livingTeamMembers = (game, team) =>
	game.private.seatedPlayers
		.filter((player, i) => player.role && player.role.team === team && !game.publicPlayersState[i].isDead && !game.publicPlayersState[i].leftGame)
...
const buildSnapshot = game => {
	...
	return {
		...
		pylons: flappyState.pylons.map(pylon => ({ id: pylon.id, x: pylon.x, gapTop: pylon.gapTop, gapBottom: pylon.gapBottom })),
...
const birdCollides = (bird, pylons, config) => {
	...
	return pylons.some(pylon => birdX + birdWidth > pylon.x && birdX < pylon.x + pylonWidth && (bird.y < pylon.gapTop || bird.y + birdHeight > pylon.gapBottom));
};
...
const handleFlappyInput = (passport, game, data) => {
	...
	if (!flappyState || !flappyState.isActive || flappyState.status !== 'running' || game.gameState.phase !== 'flappyHitler' || !data || data.type !== 'flap') {
		return;
	}

	const playerIndex = game.publicPlayersState.findIndex(player => player.userName === passport.user);

	if (playerIndex === -1 || game.publicPlayersState[playerIndex].isDead || game.publicPlayersState[playerIndex].leftGame) {
		return;
	}
