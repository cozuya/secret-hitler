        ...
        if (cardName === "fascist" || cardName === "morgana" || cardName === "monarchist") {
          if (customGameSettings.fascistCount === 2) {
            const otherFascist = seatedPlayers.find(
              (play) =>
                play.role.team === "fascist" && play.role.cardName !== "hitler" && play.userName !== player.userName
            );
          ...
          if (customGameSettings.hitKnowsFas) {
            if (customGameSettings.fascistCount === 1) {
              const otherFascist = seatedPlayers.find(
                (player) => player.role.team === "fascist" && player.role.cardName !== "hitler"
              );
              ...
              const otherFascists = seatedPlayers.filter(
                (play) => play.role.team === "fascist" && play.userName !== player.userName
              );
                ...
                player.gameChats.push({
                  ...
                  chat: [
                    ...
                    },
                    {
                      text: `${
                        customGameSettings.fascistCount === 1
                          ? " they do not know who you are since it is the "
                          : " they know who you are except for the "
                      } `,
                    },
                    {
                      text: "monarchist",
