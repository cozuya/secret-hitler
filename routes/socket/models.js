...
module.exports.formattedUserList = (isAEM) => {
  ...
  return module.exports.userList
    .map((user) => ({
      ...
      eloOverall: Number.isFinite(user.eloOverall) ? Math.floor(user.eloOverall) : undefined,
      xpOverall: user.xpOverall ? Math.floor(user.xpOverall) : undefined,
      eloSeason: Number.isFinite(user.eloSeason) ? Math.floor(user.eloSeason) : undefined,
  ...
  return Object.keys(module.exports.games).map((gameName) => ({
    ...
    tournyStatus: (() => {
      if (games[gameName].general.isTourny) {
        if (
          games[gameName].general.tournyInfo.queuedPlayers &&
