  ...
  for (const badge of ELO_BADGES) {
    ...
    if (user.eloOverall >= elo) {
      awardBadgePrequeried(
        user,
        badgeId,
        gameJustPlayed ? `You reached ${elo} ELO in the game ${gameJustPlayed}.` : ``,
        `You reached ${elo} ELO!`
      );
    }
  }
  ...
  for (const badge of XP_BADGES) {
    ...
    if (user.xpOverall >= xp) {
      awardBadgePrequeried(
        user,
        badgeId,
        gameJustPlayed ? `You reached ${xp} XP in the game ${gameJustPlayed}.` : ``,
        `You reached ${xp} XP!`
      );
    }
  }
...
module.exports.checkBadgesAccount = (user) => {
  ...
  for (const badge of ACCOUNT_AGE_BADGES) {
    ...
    if (user.created <= moment().utc().subtract(years, "years").toDate()) {
      awardBadgePrequeried(
        user,
        badgeId,
