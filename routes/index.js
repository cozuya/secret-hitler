...
const { TRIALMODS, MODERATORS, ADMINS, EDITORS } = require('../src/frontend-scripts/constants');
const fs = require('fs');
/**
		...
		getProfile(username).then(profile => {
			if (!profile) {
				...
				Account.findOne({ username }, (err, account) => {
					...
					if (!(TRIALMODS.includes(requestingUser) || MODERATORS.includes(requestingUser) || ADMINS.includes(requestingUser) || EDITORS.includes(requestingUser))) {
